  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      const mappedProducts: Product[] = data.map((p: any) => ({
        ...
        deliveryInstructions: p.post_purchase_message || p.delivery_instructions,
        requiresUserInput: p.requires_user_input,
        userInputLabel: p.user_input_label,
        requiresPassword: p.requires_password !== false,
        customUserSeesLabel: p.customer_requirement_message,
    ...
    try {
      ...
      if (data) {
        ...
        const mappedProduct: Product = {
          ...
          image: rawData.image,
          originalPrice: rawData.original_price,
          salePrice: rawData.sale_price,
          costPrice: rawData.cost_price || 0,
          duration: rawData.duration,
          features: rawData.features,
          category: rawData.category,
          deliveryType: (rawData.delivery_type as DeliveryType) || 'CREDENTIALS',
          deliveryInstructions: rawData.post_purchase_message || rawData.delivery_instructions,
          requiresUserInput: rawData.requires_user_input,
          userInputLabel: rawData.user_input_label,
          requiresPassword: rawData.requires_password !== false,
          customUserSeesLabel: rawData.customer_requirement_message,
          customRequirementsLabel: rawData.custom_requirements_label,
          isActive: rawData.is_active ?? true,
          hasVariants: rawData.has_variants || false,
          scheduledStart: rawData.scheduled_start,
          scheduledEnd: rawData.scheduled_end,
          lowStockAlert: rawData.low_stock_alert,
          useManualStock: rawData.use_manual_stock || false,
          manualStockCount: rawData.manual_stock_count || 0,
        };
